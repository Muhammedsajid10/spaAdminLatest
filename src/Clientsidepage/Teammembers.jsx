    ...
    const handleUpdateEmployee = async (memberId, updatedData) => {
      ...
      try {
        ...
        if (!updatedData.userId) {
          throw new Error('User ID is missing for the update.');
        }

        // 1. Update user details (firstName, lastName, email, phone)
        const userUpdateRes = await api.patch(
          `/admin/users/${updatedData.userId}`,
          {
        ...
        // 2. Update employee profile details (employeeId, position, department, hireDate)
        ...
        if (userUpdateRes.status === 200 && empUpdateRes.status === 200) {
          ...
          // Handle cases where one of the calls failed but didn't throw an error
        ...
        Swal.fire('Error!', err.response?.data?.message || err.message || 'Failed to update employee.', 'error');
      } finally {
