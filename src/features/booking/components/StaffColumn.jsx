...
import { localDateKey, computeAppointmentLayout } from '@features/booking/dateUtils';
import { hasShiftOnDate, getEmployeeShiftHours } from '@features/booking/shiftUtils';
...
export const StaffColumn = ({
  ...
  const { appointmentBlocks, processedSlots } = useMemo(() => {
    ...
    Object.entries(employeeAppointments).forEach(([slotKey, appointment]) => {
      ...
      const foundService = availableServices.find(s =>
        s._id === appointment.serviceEntryId ||
        s.id === appointment.serviceEntryId ||
  ...
  }, [appointments, employee.id, currentDate, timeSlots, timeSlotHeightPx, dayKey, availableServices]);
